...
const directoryExist = 'The directory already exists and is not empty. Remove existing files and continue?';
const projectDirExist = (projectDir) => `Target directory ${green(projectDir.toLowerCase())} is not empty. Remove existing files and continue?`;
...
var getPackageManager = () => /pnpm/.test(userAgent) ? 'pnpm' : /yarn/.test(userAgent) ? 'yarn' : 'npm';
...
function renderTemplate(src, dest) {
    ...
    if (stats.isDirectory()) {
        if (path$1.basename(src) === 'node_modules')
            return;
        fs$1.mkdirSync(dest, { recursive: true });
        for (const file of fs$1.readdirSync(src)) {
            renderTemplate(path$1.resolve(src, file), path$1.resolve(dest, file));
        }
        return;
    }
    ...
    if (filename.startsWith('_')) {
        dest = path$1.resolve(path$1.dirname(dest), filename.replace(/^_/, '.'));
    }
...
function stringifyJS$1(value) {
    return stringify_2(value, null, 2);
}
function renderAppConfig(fullPath, ops) {
    const { isNeedTs } = ops;
    ...
    const content = `module.exports = ${stringifyJS$1(appDefaultConfig)}`;
    fs$1.writeFileSync(path.resolve(fullPath, 'rcapp.config.js'), content, 'utf-8');
}
...
var devDependencies = {
	...
	"@szchason/eslint-config-base": "^0.0.2",
	"@szchason/eslint-config-typescript": "^0.0.3",
	"@types/node": "^18.11.10",
...
function stringifyJS(value) {
    return stringify_2(value, (val, indent, nextStringify) => {
...
function createESLintConfig({ needsTypescript, }) {
    const pkg = { devDependencies: {} };
    const addDependency = (name) => {
        pkg.devDependencies[name] = devDependencies[name];
    };
    if (needsTypescript) {
...
function renderEslint(rootDir, { needsTypescript }) {
    ...
    const packageJsonPath = path$1.resolve(rootDir, 'package.json');
    const eslintPath = path$1.resolve(rootDir, '.eslintrc.js');
...
function init() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const { shouldOverwrite, isCurrentDir, needsTypeScript, needsRouter, needsStylelint, needsEslint, needsToolkit, } = result;
